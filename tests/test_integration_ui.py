"""Window-level integration tests for scripts/09_rerun_label_review.py,
...
def test_propagate_seed_starts_worker(lr, propagate_seeded):
    ...
    assert w.kw["seed_bbox_xyxy"] == [10, 10, 40, 30]
    assert w.kw["concept"] == "ceiling light"
    assert win._propagate_meta["track_id"] == seed_tid
...
def test_propagate_queues_behind_running_job(lr, propagate_seeded):
    win, coco, seed_ann, seed_tid = propagate_seeded
    win._start_propagate_worker(1, [0, 0, 5, 5], "ceiling light", 99, 0)
    assert len(win._sam3_queue) == 1
...
def test_propagate_frame_results_apply_seed_track(lr, propagate_seeded):
    ...
    win._on_propagate_finished(1, 3)  # lost at frame 3
    assert f"propagate T{seed_tid}: lost at frame 4" in \
        win.side.sam3_status.text()
...
def test_propagate_queued_dispatch_after_run_ends(lr, propagate_seeded):
    ...
    win._sam3_queue.append({"kind": "propagate", "start_frame_idx": 2,
                            "seed_bbox_xyxy": [0, 0, 5, 5],
                            "concept": "ceiling light", "track_id": 7,
                            "cat_id": 0})
    w3.stop()
    win._on_propagate_finished(0, -1)  # ends the run, drain via direct call
    win._start_next_queued_sam3()
