...
class ModelEvaluator:
    ...
    def __init__(self, model_path: Optional[str | Path] = None, test_data_path: Optional[str | Path] = None):
        ...
        self.model = None
        self.task_type = None  # "detect" or "segment"
    
    def set_model_path(self, model_path: str | Path):
        """Set the model path."""
        self.model_path = Path(model_path)
    
    def set_test_data_path(self, test_data_path: str | Path):
    ...
    def _detect_task_type(self, model) -> str:
        ...
        # Check model task attribute (ultralytics >= 8.x)
        ...
        # Fallback: check model name for '-seg'
    ...
    def _extract_class_names(self, results, model) -> Dict[int, str]:
        ...
        elif hasattr(results.box, 'names') and results.box.names:
            class_names = results.box.names
    ...
    def _extract_per_class_ap50(self, results_component, class_names: Dict[int, str],
                                log_callback: Optional[Callable] = None) -> Dict:
        """Extract per-class AP50 from a results component (box or seg)."""
        per_class = {}
        ap50_values = None

        # Try multiple possible attribute names for per-class AP50
        ap50_attr_names = ['ap50', 'ap50_values', 'per_class_ap50', 'ap_per_class']
        for attr_name in ap50_attr_names:
            if hasattr(results_component, attr_name):
                attr_value = getattr(results_component, attr_name)
                if attr_value is not None:
                    if hasattr(attr_value, '__iter__') and not isinstance(attr_value, (str, dict)):
                        ap50_values = list(attr_value)
                        if log_callback:
                            log_callback(f"Found per-class AP50 using attribute '{attr_name}'")
                        break
                    elif isinstance(attr_value, dict) and len(attr_value) > 0:
                        for cls_idx, ap_val in attr_value.items():
                        ...
                        if log_callback:
                            log_callback(f"Found per-class AP50 dict using attribute '{attr_name}'")
                        return per_class

        # If we found ap50_values as a list, build per_class from it
        if ap50_values is not None:
    ...
    def evaluate_unseen(
        ...
        if log_callback:
            log_callback(f"Evaluating model: {m_path}, Test data: {t_path}")
        ...
        try:
            ...
            if log_callback:
                log_callback(f"Detected task type: {self.task_type}")
            ...
            if self.task_type == 'segment' and hasattr(results, 'seg') and results.seg is not None:
                ...
                if log_callback:
                    log_callback(f"Mask metrics: mAP50={metrics['mask_mAP50']:.4f}, "
                                 f"mAP50-95={metrics['mask_mAP50_95']:.4f}")
            ...
            if log_callback:
                box_attrs = [attr for attr in dir(results.box) if not attr.startswith('_')]
                log_callback(f"Available results.box attributes: {box_attrs}")
                if hasattr(results, 'seg') and results.seg is not None:
                    seg_attrs = [attr for attr in dir(results.seg) if not attr.startswith('_')]
                    log_callback(f"Available results.seg attributes: {seg_attrs}")
            
            # Extract per-class metrics from box results
            per_class = self._extract_per_class_ap50(results.box, class_names, log_callback)
            
            # For segmentation models, also extract per-class mask AP50
            if self.task_type == 'segment' and hasattr(results, 'seg') and results.seg is not None:
                per_class_seg = self._extract_per_class_ap50(results.seg, class_names, log_callback)
                # Merge mask AP50 into per_class
                for class_name, seg_metrics in per_class_seg.items():
                    if class_name in per_class:
                        per_class[class_name]["mask_AP50"] = seg_metrics.get("AP50", 0)
                    else:
                        per_class[class_name] = {
                            "AP50": 0,
                            "mAP50": 0,
                            "mask_AP50": seg_metrics.get("AP50", 0),
                        }
            
            # If we have class_names but no per_class, initialize with zeros
            if not per_class and class_names:
                if log_callback:
                    log_callback("Warning: Could not extract per-class AP50 values, initializing with zeros")
                for cls_idx, class_name in class_names.items():
                    per_class[class_name] = {"AP50": 0.0, "mAP50": 0.0}
            
            # Debug: Log class names found
            if log_callback:
                log_callback(f"Class names found: {class_names}")
                log_callback(f"Per-class metrics extracted: {list(per_class.keys())}")
                log_callback(f"Evaluation complete. mAP50: {metrics.get('mAP50', 0):.4f}")
    ...
    def load_predictions_from_json(self, json_path: str | Path) -> List[Dict]:
        """Load predictions from JSON file."""
        with open(json_path, 'r') as f:
            return json.load(f)
    
    def load_ground_truth_from_yolo(
        self, labels_dir: str | Path, images_dir: str | Path
    ) -> List[Dict]:
...
def evaluate_unseen(model_path, test_data_path, conf_threshold=0.5, iou_threshold=0.5,
...
def compare_with_gt(predictions, ground_truth, iou_threshold=0.5,
                    progress_callback=None, status_callback=None,
                    log_callback=None, is_cancelled=None):
...
def load_ground_truth_from_yolo(labels_dir, images_dir):
    evaluator = ModelEvaluator()
